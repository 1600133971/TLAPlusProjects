...
define{
  SafeWithdrawal == 
      ...
      \/ bankBalance=BALANCE-AMOUNT /\ malloryBalance=0
      \/ bankBalance=BALANCE-AMOUNT /\ malloryBalance=AMOUNT
  Invariant == bankBalance+malloryBalance <= BALANCE    
  EndState == <> (bankBalance+malloryBalance=BALANCE
...
procedure BankWithdraw(amount) {
  ...
  UpdateBalance: \* update Mallory's bankBalance  
...
\* BEGIN TRANSLATION (chksum(pcal) = "7066a1aa" /\ chksum(tla) = "b3a2b637")
\* Parameter amount of procedure BankWithdraw at line 19 col 24 changed to amount_
...
CheckBalance(self) == /\ pc[self] = "CheckBalance"
                      /\ IF bankBalance < amount_[self]
                            ...
                            ELSE /\ pc' = [pc EXCEPT ![self] = "UpdateBalance"]
                                 /\ UNCHANGED << stack, amount_ >>
                      /\ UNCHANGED << attack, bankBalance, malloryBalance, 
                                      amount >>

UpdateBalance(self) == /\ pc[self] = "UpdateBalance"
                       /\ bankBalance' = bankBalance-amount_[self]
                       /\ pc' = [pc EXCEPT ![self] = "DispenseAmount"]
                       /\ UNCHANGED << attack, malloryBalance, stack, amount_, 
                                       amount >>
...
BankWithdraw(self) == CheckBalance(self) \/ UpdateBalance(self)
                         \/ DispenseAmount(self)
...
\* Last modified Sat Jan 02 23:39:46 CST 2021 by Administrator
\* Created Sat Jan 02 23:38:40 CST 2021 by Administrator
